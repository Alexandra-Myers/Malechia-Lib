...
public class AxeItem extends MiningToolItem {
    ...
    public AxeItem(int attackDamage, float attackSpeed, ToolMaterial material, Settings settings) {
    ...
    private Optional<BlockState> getStrippedState(BlockState state) {
        return Optional.ofNullable((Block)STRIPPED_BLOCKS.get(state.getBlock())).map((block) -> {
            return (BlockState)block.getDefaultState().with(PillarBlock.AXIS, (Axis)state.get(PillarBlock.AXIS));
        });
    }
